...
- 永続化は `src/storage.ts` の `StorageAdapter`（互換名 `StoreAdapter`）越しに行う。
- `StorageAdapter` はアプリ側の境界で、保存キー1つに対する `get`、`set`、`remove`
  だけを公開する。iOS/Android ではこの IF を直接実装し、各プラットフォームの
  ローカル保存へ接続する。
- `src/storage.ts` は Chrome API を参照しない。`storage` を使う前に、
  各プラットフォームの起動境界で `configureStorageAdapter(...)` を呼び出す。
- Chrome 拡張では `src/chrome-storage.ts` の
  `configureChromeStorageAdapter()` を UI 層の初期化時に呼び、Chrome の
  map 形式 API を `StorageAdapter` に変換する。
